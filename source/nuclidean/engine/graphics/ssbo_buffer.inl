#include <engine/graphics/ssbo_buffer.h>
#include <common.h>

#include <glad/glad.h>

#include <algorithm> // std::min
#include <utility>   // std::move

namespace nc
{

//==============================================================================
template<typename T>
inline SSBOBuffer<T>::SSBOBuffer(size_t capacity)
  : m_capacity(capacity)
{
  glGenBuffers(1, &m_handle);
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_handle);
  glBufferData(GL_SHADER_STORAGE_BUFFER, capacity * sizeof(T), nullptr, GL_DYNAMIC_DRAW);
}

//==============================================================================
template<typename T>
inline void SSBOBuffer<T>::resize(size_t new_capacity)
{
  m_capacity = new_capacity;
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_handle);
  glBufferData(GL_SHADER_STORAGE_BUFFER, m_capacity * sizeof(T), nullptr, GL_DYNAMIC_DRAW);
}

//==============================================================================
template<typename T>
inline void SSBOBuffer<T>::clear()
{
  m_gpu_size = 0;
  m_size = 0;
  m_buffer.clear();
}

//==============================================================================
template<typename T>
inline size_t SSBOBuffer<T>::push_back(T&& value)
{
  // `value` is a named rvalue-reference parameter, so it is itself an
  // lvalue in this expression -- without std::move this called the copy
  // overload below instead of actually moving.
  m_buffer.push_back(std::move(value));
  m_size++;

  return m_size - 1;
}

//==============================================================================
template<typename T>
inline size_t SSBOBuffer<T>::push_back(const T& value)
{
  m_buffer.push_back(value);
  m_size++;

  return m_size - 1;
}

//==============================================================================
template<typename T>
inline void SSBOBuffer<T>::extend(std::vector<T>&& elements)
{
  m_buffer.insert
  (
    m_buffer.end(), 
    std::make_move_iterator(elements.begin()), 
    std::make_move_iterator(elements.end())
  );
  m_size += elements.size();
}

//==============================================================================
template<typename T>
inline void SSBOBuffer<T>::update_gpu_data(bool reset_capacity)
{
  // Clamp to what actually still fits -- without this, once m_gpu_size +
  // m_buffer.size() exceeds m_capacity, glBufferSubData would write past
  // the end of the GL buffer (GL_INVALID_VALUE) and the upload is silently
  // discarded with no assert or log (e.g. more than
  // MAX_VISIBLE_POINT_LIGHTS light sightings in a portal-heavy scene).
  size_t max_additions = m_capacity - std::min(m_gpu_size, m_capacity);
  size_t num_additions = std::min(max_additions, m_buffer.size());

  if (num_additions < m_buffer.size())
  {
    nc_warn
    (
      "SSBOBuffer::update_gpu_data: dropping {} of {} pending item(s) -- "
      "capacity ({}) exceeded.",
      m_buffer.size() - num_additions, m_buffer.size(), m_capacity
    );
  }

  glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_handle);
  glBufferSubData
  (
    GL_SHADER_STORAGE_BUFFER,
    m_gpu_size * sizeof(T),
    num_additions * sizeof(T),
    m_buffer.data()
  );

  m_gpu_size += num_additions;
  m_size = m_gpu_size;
  m_buffer.clear();
  if (reset_capacity)
  {
    m_buffer.shrink_to_fit();
  }
}

//==============================================================================
template<typename T>
template<typename UnaryOp>
inline void SSBOBuffer<T>::update_gpu_data(UnaryOp op, bool reset_capacity)
{
  for (T& element : m_buffer)
  {
    op(element);
  }

  update_gpu_data(reset_capacity);
}

//==============================================================================
template<typename T>
inline void SSBOBuffer<T>::update_gpu_data_with(const std::vector<T>& elements)
{
  size_t max_additions = m_capacity - std::min(m_gpu_size, m_capacity);
  size_t num_additions = std::min(max_additions, elements.size());

  glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_handle);
  glBufferSubData
  (
    GL_SHADER_STORAGE_BUFFER,
    m_gpu_size * sizeof(T),
    num_additions * sizeof(T),
    elements.data()
  );

  m_size += num_additions;
  m_gpu_size = m_size;
  nc_assert(m_gpu_size <= m_capacity);
}

//==============================================================================
template<typename T>
inline T& SSBOBuffer<T>::get_buffer_item(size_t index)
{
  return m_buffer[index];
}

//==============================================================================
template<typename T>
inline void SSBOBuffer<T>::update_gpu_item(size_t index, const T& value)
{
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_handle);
  glBufferSubData
  (
    GL_SHADER_STORAGE_BUFFER,
    index * sizeof(T),
    sizeof(T),
    &value
  );
}

//==============================================================================
template<typename T>
inline void SSBOBuffer<T>::update_gpu_item_bytes(size_t index, size_t byte_offset, const void* data, size_t byte_count)
{
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_handle);
  glBufferSubData
  (
    GL_SHADER_STORAGE_BUFFER,
    index * sizeof(T) + byte_offset,
    byte_count,
    data
  );
}

//==============================================================================
template<typename T>
inline void SSBOBuffer<T>::bind(GLuint index) const
{
  glBindBufferBase(GL_SHADER_STORAGE_BUFFER, index, m_handle);
}

//==============================================================================
template<typename T>
inline size_t SSBOBuffer<T>::capacity() const
{
  return m_capacity;
}

//==============================================================================
template<typename T>
inline size_t SSBOBuffer<T>::gpu_size() const
{
  return m_gpu_size;
}

//==============================================================================
template<typename T>
inline size_t SSBOBuffer<T>::size() const
{
  return m_size;
}

//==============================================================================
template<typename T>
inline u32 SSBOBuffer<T>::capacity_u32() const
{
  return cast<u32>(capacity());
}

//==============================================================================
template<typename T>
inline u32 SSBOBuffer<T>::gpu_size_u32() const
{
  return cast<u32>(gpu_size());
}

//==============================================================================
template<typename T>
inline u32 SSBOBuffer<T>::size_u32() const
{
  return cast<u32>(size());
}

}