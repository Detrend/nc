...
static var SECTOR_PREFAB : PackedScene:
	get: return load("res://prefabs/Sector.tscn")
static var TRIANGULATED_MULTISECTOR_PREFAB : PackedScene:
...
func is_root()->bool:
	return self.is_inside_tree() and (get_tree().edited_scene_root == self)
...
## Perform export of the whole level to a JSON file
func export_level():
	LevelExporter.new().do_export(self)
	

## Get all active [Sector]s contained in this [Level] (even autogenerated ones like children of [TriangulatedMultisector])
##
## @param for_export_only if [constant true], exclude [Sector]s that are to be excluded from export
func get_sectors(for_export_only:bool = true) -> Array[Sector]:
	...
	ret = NodeUtils.get_children_by_predicate(self, func(n:Node)->bool: return n is Sector and (n as Node2D).is_visible_in_tree() and (!for_export_only or !(n as Sector).exclude_from_export) , ret, flags)
	return ret
...
func get_editable_polygons() -> Array[EditablePolygon]:
	...
	ret = NodeUtils.get_children_by_predicate(self, func(n:Node)->bool: return n is EditablePolygon and (n as EditablePolygon).is_visible_in_tree and (n as EditablePolygon).is_editable, ret, NodeUtils.LOOKUP_FLAGS.RECURSIVE)
	return ret
...
## Perform sanity checks that apply to the [Level] as a whole
## @see [Sector.sanity_check_all]
...
# This section contains stuff related to handling user input, sending input events to objects contained in the [Level] 
## and performing some special actions - e.g. creating a new sector on Ctrl+Shift+MouseClick 
...
## Find parent where to place a new sector based on its nearest found neighbour
func _find_parent_for_new_sector(nearest_point : SectorPoint)->Node2D:
...
## Generate random name for a new sector
func _generate_random_sector_name(prefab: Resource)->String:
...
## Carries data about the action of creating a new [EditablePolygon]
class AddSectorCommand:
	var parent : Node               ## Where to put the new sector in the level hierarchy
	var child_idx : int = -1        ## At what position in the parent to put the new sector
	var prefab : PackedScene        ## Prefab for creating the new sector (can be anything derived from [EditablePolygon])
	var name: String		           ## Name for the new sector
	var position: Vector2           ## Where to put the new sector in the game world 
	var points: PackedVector2Array  ## Points to assign to the new sector
	var data: SectorProperties      ## Config for the new sector


## Generate the sector creation command based on the world position where the sector shall be placed
func make_add_sector_command(prefab: PackedScene, sector_position: Vector2, points: PackedVector2Array, sector_name: String = "", parent: Node2D = null)->AddSectorCommand:
	var ret := AddSectorCommand.new()
	var nearest_point := find_nearest_point(sector_position, INF, null)
	assert( (!nearest_point) or (nearest_point._sector is Sector))
	ret.parent = (parent if parent else _find_parent_for_new_sector(nearest_point))
	ret.prefab = prefab
	ret.name = sector_name if not sector_name.is_empty() else _generate_random_sector_name(prefab)
	ret.position = sector_position
	ret.points = points
	ret.data = (nearest_point._sector as Sector).data.duplicate() if nearest_point else null ## if there is a sector nearby, copy its config for this sector
	return ret

## Apply the sector creation command - add a new [EditablePolygon] into this [Level]
func add_sector(command: AddSectorCommand, unre: EditorUndoRedoManager = null)->EditablePolygon:	
	...
	if command.data != null:
		new_sector.set_data(command.data)
	return new_sector
...
func _handle_input()->void:
	...
	for node in selected: 
		if node is EditablePolygon: (node as EditablePolygon)._on_selected_input(selected)
