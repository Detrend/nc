...
def load_triangles(path):
    ...
    with open(path) as f:
        for line in f:
            ...
            id_part, tri_part = line.split(":", 1)
            rid = int(id_part)
            tri = ast.literal_eval(tri_part)

            triangles.append((rid, tri))
...
class TriangleViewer:
    ...
    def __init__(self, triangles):
        ...
        for rid, tri in triangles:
            ...
            self.id_to_patches[rid].append(p)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python triangles.py triangles.txt")
        return

    triangles = load_triangles(sys.argv[1])
    viewer = TriangleViewer(triangles)
